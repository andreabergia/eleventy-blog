...
A warning before you read: this is the earliest part of the project and, since [I have written this project to learn Rust]( {% ref "2023-07-12-rjvm-1.markdown" %} ), it is also the one that contains the least idiomatic code. Don't take this as an example of the best Rust ever written! 😅
...
Notice also that, since I have not modeled explicitly all the attributes that a method can have, I have opted to store a vector with the raw bytes of the [attributes](https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-4.html#jvms-4.7). Notice the "TODO" above it to remind me that I should remove it! 😅
...
where _here_ I have used some [newtype](https://doc.rust-lang.org/rust-by-example/generics/new_types.html)! 😊 `LineNumberTable` includes a simple method `lookup_pc(&self, pc: ProgramCounter) -> LineNumber` that is implemented via a binary search.
