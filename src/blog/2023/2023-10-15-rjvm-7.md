...
In this post, I discuss how I have modelled objects in [RJVM](https://github.com/andreabergia/rjvm) - we'll see a bit of lower-level code, with some pointer fiddling! 🤓 Again, if you haven't read the previous parts, some of this discussion might be hard to follow, so use the links above to check them out!
...
For storing the fields' values, I have simply decided to use 8 bytes for each field - which obviously wastes memory, because an `int` would fit in 4 bytes, a `boolean` in 1 ,etc…  but, again, I wanted to keep things relatively simple. In this way, computing a field's offset is simply a matter of [some trivial pointer arithmetic](https://github.com/andreabergia/rjvm/blob/main/vm/src/abstract_object.rs#L319):
...
And that's it for this part - and for the explanation of how RJVM works! 🎉 In the next (and last!) part, I will do a bit of retrospective about the project and my experience with Rust.

Once more, thanks a lot for reading! 😊
